...
//			System.out.println(YahooFinance.getLatestStockData("0050.TW", TimeUnit.Y, 2)); // 最近二年的每日交易紀錄
	...
	private static void validateRange(String range) throws RangeFormatException{
		...
		if (range == null) throw new RangeFormatException(String.format("range is null", null));
		
		if (range.length() < 2) throw new RangeFormatException(String.format("range is invalid.", null));
	 ...
	 * @throws RangeFormatException
	 */
	public static String getLatestStockData(String stockNo, TimeUnit unit, int number) throws IOException, RangeFormatException {
